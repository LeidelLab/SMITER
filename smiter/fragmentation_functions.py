...
class NucleosideFragmentor(AbstractFragmentor):
    ...
    def __init__(self, nucleotide_fragment_kb: Dict[str, dict] = None, raise_error_for_non_existing_fragments=True):
        """Summary."""
        ...
        self.raise_error_for_non_existing_fragments = raise_error_for_non_existing_fragments
    ...
    def fragment(self, entities: Union[list, str]):
        ...
        for entity in entities:
            if raise_error_for_non_existing_fragments is True:
                ...
                masses = self.nuc_to_fragments.get(entities, [])
            m.extend(masses)
            # logger.debug(masses)
